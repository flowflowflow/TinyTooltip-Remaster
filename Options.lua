...
function widgets:colorpick(parent, config)
    ...
    if (config.colortype == "hex") then
        ...
        local color = GetVariable(config.keystring) or {1, 1, 1, 1}
...
local saframe = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")
saframe:Hide()
...
RegisterAddOnCategory(frame)
RegisterAddOnCategory(framePCScrollFrame, frame)
RegisterAddOnCategory(frameNPCScrollFrame, frame)
RegisterAddOnCategory(frameStatusbar, frame)
RegisterAddOnCategory(frameSpell, frame)
RegisterAddOnCategory(frameFont, frame)
RegisterAddOnCategory(frameVariables, frame)
